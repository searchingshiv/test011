...
async def rclone_driver(userMess:Message,cb:CallbackQuery, merged_video_path):
	...
	DRIVE_NAME = open(conf_path,'r').readlines()[0].removesuffix(']\n').removeprefix('[')
	BASE_DIR = '/'
	edtime = 5
	try:
...
async def rclone_upload(
	merged_video_path:str,userMess:Message,cb:CallbackQuery,mess:Message,DRIVE_NAME,BASE_DIR,edTime,conf_path:str,task:RCUploadTask
):
	a=1
...
async def rclone_process_display(process:subprocess.Popen, edit_time, msg:Message, mess:Message, userMess:Message, task:RCUploadTask):
	blank = 0 #
	sleeps = False #
...
async def getGdriveLink(
	...
	# piping only stdout
	process = await asyncio.create_subprocess_exec(*get_id_cmd,stdout=asyncio.subprocess.PIPE)
